...
public class AI1 {
	 ...
	 * 电脑落棋
	 */
	public void chess(chess[][] chess) {
		int chessIndex[] = forEach(chess);// 获取AI判断的下棋位置
		chess[chessIndex[0]][ chessIndex[1]].color=1;;// 将棋子放入棋盘指定位置
	}
	 ...
	 * @return 落棋点坐标数组
	 */
	private int[] forEach(chess[][] chess) {
		int x = 0, y = 0;// 将要下的棋子坐标
		int threat = 0;// 棋盘上出现的最大威胁值
		//byte[][] chessmanArray = chess// 获得棋盘
		for (int i = 0; i < 15; i++) {// 遍历棋盘行
			for (int j = 0; j < 15; j++) {// 遍历棋盘列
				if (chess[i][j].color== 1) {// 如果此处有白棋子
					int tmp[] = catchChessModle(i, j, chess);// 捕捉每个棋子形成的棋型
	 ...
	 */
	private int[] catchChessModle(int x, int y, chess[][] chess/**chess[][] chess**/) {
		...
		// 把参数点放入每行的中心部位
		model[0][5] = model[1][5] = model[2][5] = model[3][5] = chess[x][y].color;
		// 以该棋子为中心，向两边走5步
		for (int i = 1; i <= 5; i++) {
			// 水平方向棋型
			if (x - i >= 0) {// 如果没有走出边界
				model[0][5 - i] = chess[x - i][y].color;// 将左侧棋子记录到水平棋型当中
			}
			if (x + i <= 14) {// 如果没有走出边界
				model[0][5 + i] = chess[x + i][y].color;// 将右侧棋子记录到水平棋型当中
			}

			// 垂直方向棋型
			if (y - i >= 0) {// 如果没有走出边界
				model[1][5 - i] = chess[x][y - i].color;// 将上方棋子记录到垂直棋型当中
			}
			if (y + i <= 14) {// 如果没有走出边界
				model[1][5 + i] = chess[x][y + i].color;// 将下方棋子记录到垂直棋型当中
			}

			// 反斜杠方向棋型
			if (x - i >= 0 && y + i <= 14) {// 如果没有走出边界
				model[2][5 - i] = chess[x - i][y + i].color;// 将左下方棋子记录到反斜棋型当中
			}
			if (x + i <= 14 && y - i >= 0) {// 如果没有走出边界
				model[2][5 + i] = chess[x + i][y - i].color;// 将右上方棋子记录到反斜棋型当中
			}

			// 正斜杠方向棋型
			if (x - i >= 0 && y - i >= 0) {// 如果没有走出边界
				model[3][5 - i] = chess[x - i][y - i].color;// 将左上方棋子记录到正斜棋型当中
			}
			if (x + i <= 14 && y + i <= 14) {// 如果没有走出边界
				model[3][5 + i] = chess[x + i][y + i].color;// 将右下方棋子记录到正斜棋型当中
			}
		}
